<html>
	...
	<body>
		...
		if(!empty($_POST["name"]) && !empty($_POST["comment"])) {	//投稿処理分岐
		 ...
		 function Delete() {
			 ...
			 echo "a";
			 	$filename = "mission_3-5.txt";	//ファイル変数の定義
		 		if($count >= $_POST["deleteNo"]) {
		 			echo "b";
			 		$editAr = file($filename);
			 		for($j=0; $j < $count; $j++) {//行数分だけループ
						$eA = explode("<>",$editAr[$j]); //配列を分割
						if($eA[0] == $_POST["deleteNo"]) {//編集番号と投稿番号が一致したとき
	...
	?>
		...
		<form action="mission_3-5aaa.php" method="post">
		
【入力フォーム】<br/>
		
		名前：<input type="text" name="name" value="<?php echo $nameE; ?>"><br/>
		コメント：<input type="text" name="comment" value="<?php echo $commentE; ?>"><br>
		パスワード：<input type="text" name="pass_post" >
